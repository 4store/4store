...
for $t (@tests) {
	...
	$kb_name =~s/-/_/g;
	printf(".... $t\r");
	$ret = system("FORMAT=text LANG=C ${pre} scripts/$t $kb_name > $outdir/$t $errout");
	if ($ret == 2) {
		printf("\n");
		system("./test-stop.sh $kb_name");
		system("../../utilities/4s-backend-destroy $kb_name 2>/dev/null");
		exit(2);
	}
	my $destroy = 1;
	if ($test) {
		#system("../../backend/slistdump --check s /var/lib/4store/$kb_name/*/s.slist >> $outdir/$t");
	...
	system("../../utilities/4s-backend-destroy $kb_name 2>/dev/null") if $destroy;
}
