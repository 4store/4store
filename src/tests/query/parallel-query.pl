...
$tlidq = "../4s-query $kb -f text 'PREFIX vocab: <http://www.census.gov/tiger/2002/vocab#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> SELECT * WHERE { ?line rdf:type vocab:Line } LIMIT $its OFFSET $offset' |";
open(QTEST, $tlidq);
...
$then = time();
open(QPROC, "| ../4s-query $kb -P > /dev/null");
for $i (0..$its-1) {
	sleep(0.001) if $i > 0;
